...
| Schedules | Named cards «Сон» 21:00–07:00 all days, «Учёба» 08:00–16:00 Пн–Пт, toggle, day chips; «Добавить расписание» (name, start, end, days). Blocks the whole pool | Same model (`QuietInterval`) **plus a per-schedule app list** (`packages`, owner 04.09.2026): a schedule closes only the apps picked for it, never the whole phone; no apps = blocks nothing. List = icon card + switch + «N приложений»; editor = name, two clock drums «Начало»/«Конец», day chips, «Какие закрывать» → full-screen picker (essentials and «Доступно всегда» not selectable). Presets «Сон»/«Учёба» open the editor pre-filled |
