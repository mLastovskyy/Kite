...
/** The single latest position of a child, for the parent's live map. */
@Serializable
data class DeviceLocationRow(
...
/**
 * Upserts / reads the latest child position over PostgREST. Only this one row per member is
 * synced for the live map; the full point history stays in Room on the child (CLAUDE.md).
 */
class DeviceLocationRemote(
    ...
    suspend fun upsert(row: DeviceLocationRow): Result<Unit> = runCatching {
        val response =
            httpClient.post("$restUrl/device_location") {
    ...
    suspend fun latest(memberId: String): Result<DeviceLocationRow?> = runCatching {
        val response =
            httpClient.get("$restUrl/device_location") {
                authHeaders(requireSession())
                parameter("member_id", "eq.$memberId")
                parameter(
                    "select",
                    "member_id,family_id,latitude,longitude,accuracy_m,battery_pct,recorded_at",
                )
            }
        if (!response.status.isSuccess()) throw restError(response)
        json.decodeFromString<List<DeviceLocationRow>>(response.bodyAsText()).firstOrNull()
    }.mapNetworkError()
